...
Usage:
    python fable5_train_sac.py                                  # frozen fast path
    python fable5_train_sac.py finetune.enabled=true finetune.encoder_grads=critic \
        env.image_size=112 embedding=resnet18
    python fable5_train_sac.py finetune.enabled=true finetune.encoder_grads=none \
        env.image_size=112                                      # slow-frozen protocol
    python fable5_train_sac.py finetune.enabled=true finetune.aux_loss=l2sp \
        finetune.encoder_grads=none env.image_size=112          # aux-only adaptation
    python fable5_train_sac.py env=pendulum embedding=none      # state-based sanity check
"""
...
def main(cfg: DictConfig) -> None:
    ...
    if "finetune" not in cfg:
        raise KeyError(
            "cfg.finetune missing -- merge the `finetune:` block from "
            "config_finetune_snippet.yaml into configs/config_sac_fable5.yaml "
